...
here::here()
exp <- read.csv(here::here("data/mouse_ALM_2018-06-14_exon-matrix.csv"),row.names = 1,header = T)
meta <- read.csv(here::here("data/mouse_ALM_2018-06-14_samples-columns.csv"), row.names = 1, header = T)
geneinfo <- read.csv(here::here("data/mouse_ALM_2018-06-14_genes-rows.csv"), row.names = 4, header = T)
...
L5ALM.ITPT <- QuickSeurat(L5ALM.ITPT)

pdf("clusters.pdf",width = 20,height = 10)
...
allmarkers.subclass = FindAllMarkers(object = L5ALM.ITPT, logfc.threshold = 0.1,min.pct = 0.05,only.pos = T)

head(allmarkers.subclass)
library(plotly)
p <- ggplot(allmarkers.subclass, aes(avg_logFC, y=-1*log10(p_val_adj), color=cluster ))+
